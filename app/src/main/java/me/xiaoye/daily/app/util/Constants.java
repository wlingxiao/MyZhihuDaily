package me.xiaoye.daily.app.util;

/**
 * Created by VAIO on 2016/4/8.
 */
public class Constants {
    public final static String TEST = "<h3>你好世界</h3>";
    public final static String ZHIHU_LATEST = "http://news-at.zhihu.com/api/4/news/latest";
    public final static String ZHIHU_OLD = "http://news.at.zhihu.com/api/4/news/before/";
    public final static String ZHIHU_CONTENT = "http://news-at.zhihu.com/api/4/news/";
    public final static String ZHIHU_THEMES= "http://news-at.zhihu.com/api/4/themes";
    public final static String ZHIHU_THEMES_CONTENT= "http://news-at.zhihu.com/api/4/theme/";

}
